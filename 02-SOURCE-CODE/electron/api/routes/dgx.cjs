 ...
 *   POST /api/dgx/kill-process/:id - Kill a process by PID
 *
 * Project Management:
...
// Kill a process on DGX
router.post('/kill-process/:id', async (req, res) => {
  try {
    const { id } = req.params;
    ...
    // Validate signal
    const validSignals = ['TERM', 'KILL', 'INT', 'QUIT', 'HUP'];
    ...
    const command = `kill -${signal.toUpperCase()} ${pid}`;
    const result = await dgxManager.executeCommand(id, command);
